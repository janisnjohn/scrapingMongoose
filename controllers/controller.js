...
console.log("scraping");
  ...
  request('http://www.abc15.com/yuma', function(error, response, html) {
    ...
    $('article .inner').each(function(i, element) {
        ...
        result.title = $(this).children('h6').children('a').text().trim() + ""; //convert to string for error handling later

        // Collect the Article Link (contained within the "a" tag of the "h2" in the "header" of "this")
        result.link = 'http://www.abc15.com/yuma' + $(this).children('h6').children('a').attr('href');
